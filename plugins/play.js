...
const handler = async (m, {
    ...
    if (!text) throw `*_هات اسم الاغنيه_* \n*_مثال: ${usedPrefix + command} طويل الشوق_*`;
    conn.GURUPLAY = conn.GURUPLAY ? conn.GURUPLAY : {};
    await conn.reply(m.chat, wait, m);
    const result = await searchAndDownloadMusic(text);
    const infoText = `🎧🎼『 *𝑮𝑶𝑲𝑼 𝑩𝑶𝑻 - غـــوكـــو* 』🎼🎧

    
   
*🎶اكتب رقم عشان يجيلك الاغنيه🎵*`;
...
handler.before = async (m, {
    ...
    let title = generateRandomName();
        ...
        m.reply("رقم غلط اختار رقم من القايمه.\nبين 1 و " + result.allLinks.length);
    }
};

handler.help = ["شغل"];
handler.tags = ["downloader"];
handler.command = /^(play|2شغل2|اغنيه)$/i;
handler.limit = false;
...
async function searchAndDownloadMusic(query) {
    try {
        ...
        if (!videos.length) return "لم يتم الحصول علي هذا البحث.";
