...
const handler = async (m, {conn, args, command, usedPrefix}) => {
  if (!args[0]) throw `*[❗اشعار❗] يرجى إرسال رابط منشور إنستجرام. مثال: ${usedPrefix + command} https://www.instagram.com/mrwbryh?igsh=MWxwZ2o4N2NkMHN5YQ==*`;
  m.reply(global.wait);
  try {
...
for (let i = 0; i < img.length; i++) {
  ...
  try {
    ...
    for (const urRRl of datTa) {
      ...
      const tXXxt = `🔗 *لينك:* ${shortUrRRl}`.trim();
      ...
      try {
        ...
        const txt2 = `🔗 *لينك:* ${shortUrl2}`.trim();
        await conn.sendFile(m.chat, resultss, 'error.mp4', txt2, m);
      } catch {
        try {
          const resultssss = await instagramdl(args[0]);
          const shortUrl3 = await (await fetch(`https://tinyurl.com/api-create.php?url=${args[0]}`)).text();
          const txt4 = `🔗 *لينك:* ${shortUrl3}`.trim();
          ...
          try {
            ...
            const txt1 = `🔗 *لينك:* ${shortUrl1}`.trim();
            await conn.sendFile(m.chat, videoig, 'error.mp4', txt1, m);
          } catch {
            throw `*[❗اشعار❗] خطأ، لا يمكن التحميل من الفيديو أو الصوت*`;
          }
        }
      }
    }
...
handler.command = /^(instagramdl|2انستجرام2|انستا|ig2|instagramdl2|instagram2|igdl2|ig2|instagramdl3|instagram3|igdl3|ig3)$/i;
